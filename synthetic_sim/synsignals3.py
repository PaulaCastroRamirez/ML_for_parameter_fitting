...
# -*- coding: utf-8 -*-
"""
Created on Fri Oct 13 14:33:54 2023
...
# =============================================================================
    ...
    # Parameters
	kappa = 250 
	mu = qnet.param_max[0]/2 
	mu1 = qnet.param_min[0] 
	kappa1 = 2.0  
	mu2 = qnet.param_max[0] 
	kappa2 = 2.0  

	pp=0
	train = np.concatenate((vonmises.rvs(kappa, loc=mu, size=int((0.08)*ntrain)),vonmises.rvs(kappa1, loc=mu1, size= int((0.46)*ntrain)),vonmises.rvs(kappa2, loc=mu2, size=int((0.46)*ntrain))))
	ptrain[:,pp] = (train + qnet.param_max[0] / 2) % qnet.param_max[0]
	val = np.concatenate((vonmises.rvs(kappa, loc=mu, size=int((0.08)*nval)),vonmises.rvs(kappa1, loc=mu1, size= int(0.46*nval)),vonmises.rvs(kappa2, loc=mu2, size=int(0.46*nval))))
	pval[:,pp] = (val + qnet.param_max[0] / 2) % qnet.param_max[0]
	test = np.concatenate((vonmises.rvs(kappa, loc=mu, size=int((0.08)*ntest)),vonmises.rvs(kappa1, loc=mu1, size= int(0.46*ntest)),vonmises.rvs(kappa2, loc=mu2, size=int(0.46*ntest))))
	ptest[:,pp] = (test + qnet.param_max[0] / 2) % qnet.param_max[0]


    ### theta (0 to 2pi) ###
    # Parameters
	kappa1 = 70
	mu1 = qnet.param_max[1]/2 
	mu2 = 0.0  
	kappa2 = 0.5  
	mu3 = qnet.param_max[1]/2 
	kappa3 = 0.5  

	pp=1
	train = np.concatenate((vonmises.rvs(kappa1, loc=mu1, size=int((0.3)*ntrain)), vonmises.rvs(kappa2, loc=mu2, size=int(0.35*ntrain)), vonmises.rvs(kappa3, loc=mu3, size=int(0.35*ntrain))))
	ptrain[:,pp] = (train + qnet.param_max[1]/2) % (qnet.param_max[1])
	val = np.concatenate((vonmises.rvs(kappa1, loc=mu1, size=int((0.3)*nval)), vonmises.rvs(kappa2, loc=mu2, size=int(0.35*nval)), vonmises.rvs(kappa3, loc=mu3, size=int(0.35*nval))))
	pval[:,pp] = (val + qnet.param_max[1]/2) % (qnet.param_max[1])
	test = np.concatenate((vonmises.rvs(kappa1, loc=mu1, size=int((0.3)*ntest)), vonmises.rvs(kappa2, loc=mu2, size=int(0.35*ntest)), vonmises.rvs(kappa3, loc=mu3, size=int(0.35*ntest))))
	ptest[:,pp] = (test + qnet.param_max[1]/2) % (qnet.param_max[1])
	...
	pp=2
	ptrain[:,pp]  = np.concatenate((np.random.normal(mean, std_dev, int(0.8*ntrain)), np.random.uniform(0.9,  3.1, int(0.2*ntrain)) ))
	pval[:,pp]  = np.concatenate((np.random.normal(mean, std_dev, int(0.8*nval)), np.random.uniform(0.9,  3.1, int(0.2*nval)) ))
	ptest[:,pp]   = np.concatenate((np.random.normal(mean, std_dev, int(0.8*ntest)), np.random.uniform(0.9,  3.1, int(0.2*ntest)) ))
	...
	pp=3
	train = (vonmises.rvs(kappa4, loc=mu4, size=int(0.10*ntrain)) + mu4) % (2 * mu4)
	ptrain[:,pp]  = np.concatenate((np.random.normal(mean1, std_dev1, int(0.5*ntrain)), np.random.uniform(0,  3.1, int(0.20*ntrain)), np.random.normal(mean2, std_dev2, int(0.20*ntrain)), train))
	val = (vonmises.rvs(kappa4, loc=mu4, size=int(0.10*nval)) + mu4) % (2 * mu4)
	pval[:,pp]  = np.concatenate((np.random.normal(mean1, std_dev1, int(0.5*nval)), np.random.uniform(0,  3.1, int(0.20*nval)), np.random.normal(mean2, std_dev2, int(0.20*nval)), val))
	test = (vonmises.rvs(kappa4, loc=mu4, size=int(0.10*ntest)) + mu4) % (2 * mu4)
	ptest[:,pp]   = np.concatenate((np.random.normal(mean1, std_dev1, int(0.5*ntest)), np.random.uniform(0,  3.1, int(0.20*ntest)), np.random.normal(mean2, std_dev2, int(0.20*ntest)), test))
	...
	pp=4
	ptrain[:,pp]  = skewnorm.rvs(a=skewness, loc=center, scale=std_dev, size=int(ntrain))
	pval[:,pp] = skewnorm.rvs(a=skewness, loc=center, scale=std_dev, size=int(nval))
	ptest[:,pp] = skewnorm.rvs(a=skewness, loc=center, scale=std_dev, size=int(ntest))
